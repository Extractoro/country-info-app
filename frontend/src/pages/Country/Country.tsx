...
const Country = () => {
    ...
    if (
        !data ||
        !data.populationData ||
        !Array.isArray(data.populationData.data.populationCounts)
    ) {
    ...
    const chartData = {
        labels: country.populationData.data.populationCounts.map(
            (data) => data.year,
        ),
        datasets: [
            {
                label: 'Population',
                data: country.populationData.data.populationCounts.map(
                    (data) => data.value,
                ),
                fill: false,
        ...
        <div className="p-5 max-w-4xl mx-auto">
            <div className="flex items-center mb-8">
                <img
                    src={country.flagUrl}
                    alt={country.name}
                    className="w-32 h-20 mr-5 shadow-2xl"
                />
                    ...
                    {country.borderCountries.length > 0 ? (
                        country.borderCountries.map((border) => (
                            <li key={border.countryCode} className="py-2">
                                <Link
                                    to={`/country/${border.countryCode}/${border.commonName}`}
